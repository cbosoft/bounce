// Based on "MattiasCRT" on shadertoy: https://www.shadertoy.com/user/Mattias
// Which, in turn, is based on a postprocessing shader by inigo quilez
...
void main()
{
    vec2 q = TexCoords; //gl_FragCoord.xy / window_size;
    vec2 uv = q;
    uv = curve( uv );
    vec3 oricol = texture(screenTexture, q.xy).xyz;
    vec3 col;
    ...
    float scans = clamp( 0.35+0.35*sin(3.5*time+uv.y*window_size.y*1.5), 0.0, 1.0);
